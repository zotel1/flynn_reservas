...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    for (const k of ['GOOGLE_CLIENT_ID','GOOGLE_CLIENT_SECRET','GOOGLE_REDIRECT_URI','GOOGLE_REFRESH_TOKEN','ENCARGADO_EMAIL']) {
      if (!process.env[k]) missing.push(k);
    }
    if (!process.env.SHEET_ID) missing.push('SHEET_ID');
    if (!process.env.CALENDAR_ID) missing.push('CALENDAR_ID (sugerido: "primary")');
    ...
    try {
      const gp = await gmail.users.getProfile({ userId: 'me' });
      out.checks.gmail = { ok:true, address: gp.data.emailAddress };
    } catch (e:any) {
